...
void get_current_ntp_time(ntp_timestamp_t *ntp_ts){
    ...
    ntp_ts->seconds = UNIX_TO_NTP_SECONDS(currentTime.tv_sec);
    ntp_ts->fraction = currentTime.tv_usec * (NTP_FRACTION_SCALE / USEC_INCREMENTS);
    // ntp_ts_to_net(ntp_ts);
}
...
void ntp_time_to_string(const ntp_timestamp_t *ntp_ts, char *buffer, size_t buffer_size, int local) {
    ...
    // seconds = ntohl(ntp_ts->seconds);
    // fraction = ntohl(ntp_ts->fraction);
    seconds = ntp_ts->seconds;
    fraction = ntp_ts->fraction;
    seconds = NTP_TO_UNIX_SECONDS(seconds);
    fraction = (fraction * USEC_INCREMENTS) / NTP_FRACTION_SCALE;
    const time_t seconds_tt = seconds;
    
    if (local == UTC_TIME) {
        local_time = gmtime(&seconds_tt);
    } else if (local == LOCAL_TIME) {
        local_time = localtime(&seconds_tt);
    }

    snprintf(buffer, buffer_size, "%04d-%02d-%02d %02d:%02d:%02d.%06.0f\n",
                local_time->tm_year + NTP_EPOCH_YEAR,
                local_time->tm_mon,
                local_time->tm_mday,
                local_time->tm_hour,
                local_time->tm_min,
...
double ntp_time_to_double(const ntp_timestamp_t* timestamp) {
    ...
    double seconds, fraction, result;

    // seconds = ntohl(timestamp->seconds);
    // fraction = (double)ntohl(timestamp->fraction) / NTP_FRACTION_SCALE;
    seconds = timestamp->seconds;
    fraction = (double)timestamp->fraction / NTP_FRACTION_SCALE;
...
void print_ntp_time(const ntp_timestamp_t *ts, const char* label, int local){
    ...
    ntp_time_to_string(ts, buffer, sizeof(buffer), local);
    buffer[TIME_BUFF_SIZE - 6] = '\0';
    if (local == LOCAL_TIME) {
        localString = LOCAL_TIME_STR;
    } else if (local == UTC_TIME) {
        localString = UTC_TIME_STR;
    }

    printf("%s: %s (%s)\n", label, buffer, localString);
}
...
int build_ntp_request(ntp_packet_t* packet) {
    ...
    packet->poll = 6;
    packet->precision = -20;
    ntp_timestamp_t xmit_time;
    get_current_ntp_time(&xmit_time);
    packet->xmit_time = xmit_time;

    // printf("XMIT TIME: %u\n", packet->xmit_time.seconds);

    // char buffer[TIME_BUFF_SIZE];
    // ntp_time_to_string(&xmit_time, buffer, sizeof(buffer), LOCAL_TIME);
    // printf("%s", buffer);
    // ntp_time_to_double(&xmit_time);

    // print_ntp_time(&xmit_time, "Transmit Time", LOCAL_TIME);

    // char buffer[16];
    // decode_reference_id(1, 0x4e495354, buffer, 16);
...
int decode_reference_id(uint8_t stratum, uint32_t ref_id, char *buff, int buff_sz){
    ...
    if (buff_sz_needed > buff_sz) return RC_BUFF_TOO_SMALL;

    if (ref_id == 0) {
        memcpy(buff, "None", buff_sz_needed);
    } else if (stratum >= 2) {
        ref_id = ntohl(ref_id);
        inet_ntop(AF_INET, &ref_id, buff, buff_sz_needed);      // Error handling maybe later
    } else if (stratum < 2) {
        ref_id = ntohl(ref_id);
        memcpy(buff, &ref_id, buff_sz_needed);
    }
...
int calculate_ntp_offset(const ntp_packet_t* request, 
    ...
    double t1, t2, t3, t4;
    double server_dispersion, server_delay;
    double offset, delay, final_dispersion;
    t1 = ntp_time_to_double(&response->orig_time);
    t2 = ntp_time_to_double(&response->recv_time);
    t3 = ntp_time_to_double(&response->xmit_time);
    t4 = ntp_time_to_double(recv_time);
    server_dispersion = GET_NTP_Q1616_TS(response->root_dispersion);
    server_delay = GET_NTP_Q1616_TS(response->root_delay);

    offset = ((t2 - t1) + (t3 - t4)) / 2;
    delay = (t4 - t1) - (t3 - t2);
    final_dispersion = server_dispersion + (server_delay / 2) + (delay / 2); 
    ...
    memcpy(&result->server_time, &response->recv_time, sizeof(ntp_timestamp_t));
...
void print_ntp_packet_info(const ntp_packet_t* packet, const char* label, int packet_type) {
    ...
    if (packet_type == IS_REQUEST) {
        packet_type_str = "Request";
    } else if (packet_type == IS_RESPONSE) {
        packet_type_str = "Response";
    }
    char buffer[REFID_SIZE];
    int ret = decode_reference_id(packet->stratum, packet->reference_id, buffer, sizeof(buffer));
    ...
    printf("Poll: %u\n", packet->poll);
    printf("Precision: %d\n", packet->precision);
    printf("Reference ID: [0x%x] %s\n", packet->reference_id, buffer);
...
void print_ntp_results(const ntp_result_t* result) {
    ...
    print_ntp_time(&result->server_time, "Server Time", LOCAL_TIME);
    print_ntp_time(&result->client_time, "Local Time", LOCAL_TIME);
    ...
    if (result->offset >= 0) {
        printf("Your clock is running BEHIND by %.2f ms\n",result->offset * 1000);
    } else{
        printf("Your clock is running ADVANCE by %.2f ms\n", result->offset * 1000);
    }
    printf("Your estimated time error will be +/- %.2f ms\n", result->final_dispersion * 1000);
}